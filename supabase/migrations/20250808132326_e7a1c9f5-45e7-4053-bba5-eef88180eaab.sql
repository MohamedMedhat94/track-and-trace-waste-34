-- Create admin user and set up system improvements (fixed)
DO $$
DECLARE
  admin_user_id UUID;
BEGIN
  -- Insert admin profile if it doesn't exist (check by email in WHERE clause)
  ...
  SELECT 
    ...
    now()
  WHERE NOT EXISTS (
    SELECT 1 FROM public.profiles WHERE email = 'Abdullah1@gmail.com'
  );
